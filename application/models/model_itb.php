<?php
class Model_itb extends CI_Model
{
    function insert_itb($data)
    {
    ...
    function get_itb()
    {
        $data = $this->db->select('id, manfaat, jenis_benefit.kuantitas, satuan.nama_satuan as satuan, tarif, jenis_benefit.keterangan')
            ->from('jenis_benefit')
            ->join('cost_benefit', 'jenis_benefit.id_cost_benefit = cost_benefit.id_cost_benefit and jenis_benefit.id_cost_benefit = 6')
            ->join('satuan', 'id_satuan = jenis_benefit.satuan');
        $data = $data->get();
        $data = $data->result();
        return $data;
    }
    function delete_itb($id)
    {
        $this->db->where('id', $id);
        $this->db->delete('jenis_benefit');
    }
    function ubah_itb($id, $update)
    {
