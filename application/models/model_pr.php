...
class Model_pr extends CI_Model
{
    ...
    function get_pr()
    {
        $data = $this->db->select('id, kebutuhan, jenis_cost.kuantitas, satuan.nama_satuan as satuan, jenis_cost.keterangan')
            ->from('jenis_cost')
            ->join('cost_benefit', 'jenis_cost.id_cost_benefit = cost_benefit.id_cost_benefit and jenis_cost.id_cost_benefit = 3')
    ...
    function insert_view_pr($data)
    {
        $this->db->insert('view_data', $data);
    }
    function delete_view_pr($id)
    {
        $this->db->where('id_data', $id);
        $this->db->delete('view_data');
    }
    function ubah_view_pr($id, $update)
    {
