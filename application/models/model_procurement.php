...
class Model_procurement extends CI_Model
{
    ...
    function get_procurement()
    {
        $data = $this->db->select('id, kebutuhan, jenis_cost.kuantitas, satuan.nama_satuan as satuan, jenis_cost.keterangan')
            ->from('jenis_cost')
            ->join('cost_benefit', 'jenis_cost.id_cost_benefit = cost_benefit.id_cost_benefit and jenis_cost.id_cost_benefit = 1')
    ...
    function get_view_proc($id)
    {
        $data = $this->db->select('id_data, deskripsi, view_data.kuantitas, satuan.id_satuan, satuan.nama_satuan as satuan, harga_satuan, view_data.keterangan, jenis_cost.kuantitas as qty')
            ->from('view_data')
    ...
    function insert_view_proc($data)
    {
        $this->db->insert('view_data', $data);
    }
    function delete_view_procurement($id)
    {
        $this->db->where('id_data', $id);
        $this->db->delete('view_data');
    }
    function ubah_view_proc($id, $update)
    {
