<?php
class Model_startup extends CI_Model
{
    function insert_startup($data)
    {
    ...
    function get_startup()
    {
        $data = $this->db->select('id, kebutuhan, jenis_cost.kuantitas, satuan.nama_satuan as satuan, jenis_cost.keterangan')
            ->from('jenis_cost')
            ->join('cost_benefit', 'jenis_cost.id_cost_benefit = cost_benefit.id_cost_benefit and jenis_cost.id_cost_benefit = 2')
    ...
    function ubah_startup($id, $update)
    {
    ...
    function insert_view_startup($data)
    {
        $this->db->insert('view_data', $data);
    }
    function delete_view_startup($id)
    {
        $this->db->where('id_data', $id);
        $this->db->delete('view_data');
    }
    function ubah_view_startup($id, $update)
    {
